...
from aag_safe_suspend import production, transaction as tx, workloads
...
class RulesTests(unittest.TestCase):
    ...
    def test_previous_boot_is_not_replayed(self):
        self.assertEqual(tx.request_disposition(tx.new('old', 'a'), 'new', 'b', False), 'BOOT_RECONCILE')
    ...
    def test_kernel_success_without_residency_is_not_low_power(self):
        evidence = tx.sleep_evidence({'success': 2, 'total_hw_sleep': 9},
                                     {'success': 3, 'total_hw_sleep': 9}, 'success')
        self.assertTrue(evidence['kernel_cycle'])
        self.assertFalse(evidence['actual_sleep'])

    def test_actual_sleep_and_resume(self):
        evidence = tx.sleep_evidence({'success': 2, 'total_hw_sleep': 9, 'boottime': 2, 'monotonic': 2},
            {'success': 3, 'total_hw_sleep': 10000009, 'boottime': 14, 'monotonic': 4}, 'success')
    ...
    def test_immediate_wake_is_distinct(self):
        e = tx.sleep_evidence({'success': 0, 'total_hw_sleep': 0},
                             {'success': 1, 'total_hw_sleep': 20}, 'success')
    ...
    def test_normal_temperature_never_powers_off_for_elapsed_retry_budget(self):
        for elapsed in (0, 900, 9000, 90000):
            action, _ = tx.safety_action({'state': 'NORMAL'}, {}, warning_seconds=elapsed,
                                         missing_seconds=elapsed, lid='closed')
    ...
    def test_actual_thermal_emergency_still_has_failsafe(self):
        ...
        self.assertEqual(tx.safety_action({'state': 'THERMAL_WARNING'}, {}, warning_seconds=59)[0], 'monitor')
        self.assertEqual(tx.safety_action({'state': 'THERMAL_WARNING'}, {}, warning_seconds=60)[0], 'poweroff')

    def test_missing_temperature_bounded_closed_lid_safety(self):
        self.assertEqual(tx.safety_action({'state': 'UNKNOWN'}, {}, missing_seconds=299)[0], 'monitor')
        self.assertEqual(tx.safety_action({'state': 'UNKNOWN'}, {}, missing_seconds=300)[0], 'poweroff')
        self.assertEqual(tx.safety_action({'state': 'UNKNOWN'}, {}, missing_seconds=999, lid='open')[0], 'monitor')


class WorkloadTests(unittest.TestCase):
    def setUp(self):
        self.row = dict(name='checkpoint worker', importance='nonessential_checkpointable',
                        detect=['/usr/bin/check'], safe_stop_command=['/usr/bin/checkpoint'],
                        verify_stopped=['/usr/bin/check'], stop_timeout=20,
                        restart_after_resume='never', storage_dependency='DATA')

    def test_checkpoint_before_suspend_and_never_auto_resume(self):
        receipts = []
        run = Mock(side_effect=[{'active': True}, {'durable_state_preserved': True}, {'active': False}])
        save = Mock()
    ...
    def test_graceful_stop_failure_never_escalates_to_kill(self):
        run = Mock(side_effect=[{'active': True}, {'durable_state_preserved': False}, {'active': True}])
        receipts = []
        with self.assertRaises(tx.Refusal):
    ...
    def test_explicit_restore_only_stopped_workload(self):
        row = {**self.row, 'restart_after_resume': 'if_was_running', 'restart_command': ['/usr/bin/resume']}
        receipts = [{'name': row['name'], 'status': 'STOPPED', 'was_running': True,
...
class HostTests(unittest.TestCase):
    def setUp(self):
        ...
        self.r = SimpleNamespace(state_lock=contextlib.nullcontext, load_state=lambda: copy.deepcopy(self.state),
             boot_id=lambda: 'boot', save_state=save, arm_automount_suppression=Mock(),
             validate_automount_marker_for_release=Mock(), log=Mock(), atomic_json=Mock(),
             live_registered_jobs=lambda: [], timeshift_processes=lambda: [],
             internal_data_healthy=lambda: (True, 'healthy'), incident=Mock(),
             audit_three=lambda: {'AUDIT_COMPLETE': True, 'EXTERNAL_OWNER': 'NONE', 'records': []},
             terminal_ugreen_reaudit=lambda v: {'ok': True}, clear_to_idle=lambda v, end: save(
                 {'state': 'IDLE', 'boot_id': 'boot', 'last_terminal': end}),
             thermal_sample=lambda: {'state': 'NORMAL'}, lid_state=lambda: 'open', unsafe_mount_topology=lambda r: [])
        self.host.r = self.r
        self.host.t = SimpleNamespace(PENDING=self.root/'pending', QUEUE=self.root/'queue',
             BLOCKED=self.root/'blocked', LAST=self.root/'last', pre=Mock(), post=Mock(return_value=0),
             restore_transaction=Mock(), read_json=lambda p: {'before': {'boot_id': 'boot'}})
    ...
    def test_successful_preparation_to_real_resume(self):
        ...
        with patch.object(production, 'stats', return_value={'success': 1, 'total_hw_sleep': 2000000}):
    ...
    def test_unknown_cwd_fd_namespace_and_docker_blockers_are_preserved(self):
        for kind in ('cwd', 'fd', 'mmap', 'root', 'mount', 'raw-fd'):
            self.r.audit_three = lambda: {'AUDIT_COMPLETE': True, 'EXTERNAL_OWNER': 'HOST',
    ...
    def test_incomplete_or_disconnected_storage_refused(self):
        for complete, owner in ((False, 'UNKNOWN'), (True, 'HOST')):
            self.r.audit_three = lambda: {'AUDIT_COMPLETE': complete, 'EXTERNAL_OWNER': owner, 'records': []}
            with self.assertRaises(tx.Refusal):
                self.host.verify_release()
            self.assertEqual(self.state['state'], 'FAILURE_PENDING')
    ...
    def test_terminal_storage_recovery_failure_not_complete(self):
        self.state['verify_invocation'] = 'prep'
        self.r.terminal_ugreen_reaudit = Mock(return_value={'ok': False, 'reason': 'generation unstable'})
        with self.assertRaises(tx.Refusal):
            self.host.resume_terminal()
        self.assertEqual(self.state['state'], 'FAILURE_PENDING')
    ...
    def test_lid_open_clears_failed_transaction_without_suspending(self):
        ...
        with patch.object(production, 'battery', return_value={}), patch.object(production, 'run') as run:
            self.host.monitor()
        self.assertEqual(self.state['state'], 'IDLE')
        run.assert_not_called()
    ...
    def test_locklock_on_cancels_automatic_retry(self):
        ...
        with patch.object(production, 'lid_ignored', return_value=True), patch.object(production, 'run') as run:
            self.host.retry()
        run.assert_not_called()
        self.assertEqual(self.state['state'], 'IDLE')
    ...
    def test_locklock_off_uses_normal_suspend_dispatch(self):
        ...
        with patch.object(production, 'lid_ignored', return_value=False), patch.object(production, 'run', side_effect=accepted) as run:
            self.host.retry()
...
class ClonePolicyTests(unittest.TestCase):
    ...
    def setup_clone(self):
        ...
        self.clone = {'name': 'usbclone_test', 'udc': 'dummy_udc.0',
                      'functions': ['mass_storage.0'], 'backings': ['/sample/disk.img']}
        self.host.cfg['usbclone'] = {'command': str(command), 'sha256': hashlib.sha256(command.read_bytes()).hexdigest(),
    ...
    def test_unused_known_clone_stops_only_after_audits(self):
        ...
        self.assertEqual(self.state['usbclone_stopped'][0]['restart_after_resume'], 'never')

    def test_live_guest_fd_prohibits_usb_stop(self):
        self.setup_clone()
        self.host.clone_audit.return_value = {'AUDIT_COMPLETE': True, 'records': [{'kind': 'usb-or-backing-fd'}]}
        with patch.object(production, 'trusted'), patch.object(production, 'run') as run:
            with self.assertRaises(tx.Refusal):production.Host.quiesce_clones(self.host)
        run.assert_not_called()
        self.assertEqual(self.state['state'], 'FAILURE_PENDING')
    ...
    def test_clean_unmount_is_never_forced_and_failure_aborts(self):
        self.setup_clone()
        self.host.clone_audit.return_value = {'AUDIT_COMPLETE': True, 'records': [
            {'kind': 'mount', 'owner': 'HOST', 'target': '/sample/mount'}]}
        with patch.object(production, 'trusted'), patch.object(production, 'run', side_effect=[None, tx.Refusal('busy')]) as run:
            with self.assertRaises(tx.Refusal):production.Host.quiesce_clones(self.host)
    ...
    def test_raw_backing_hardlink_fd_and_mmap_are_detected(self):
        ...
        proc = self.root/'proc';sysroot = self.root/'sys'
        (proc/'42/fd').mkdir(parents=True)
        (sysroot/'class/block').mkdir(parents=True)
        (sysroot/'bus/usb/devices').mkdir(parents=True)
        backing = self.root/'disk.img';backing.write_bytes(b'fixture')
        alias = self.root/'alias.img';os.link(backing, alias)
        (proc/'42/fd/7').symlink_to(alias)
        st = backing.stat()
        (proc/'42/maps').write_text(f'1000-2000 rw-s 00000000 {os.major(st.st_dev):x}:{os.minor(st.st_dev):x} {st.st_ino} {backing}\n')
        real_path = Path
        def path(value):
            if str(value)=='/proc':return proc
            if str(value).startswith('/sys/'):return sysroot/str(value)[5:]
        ...
        self.assertEqual({r['kind'] for r in result['records']}, {'usb-or-backing-fd', 'backing-mmap'})
