...
from aag_safe_suspend import workloads as w, locklock_compat as ll, transaction as tx


class ClassificationTests(unittest.TestCase):
    def info(self, **changes):
        return dict(pid=555, start='42', ppid=1, state='S', flags=0,
                    uids=[1000]*4, caps=0, exe='/tmp/unknown-future-app',
                    cmdline='unknown-future-app', cgroup='', **changes)
    ...
    def test_critical_components_excluded_even_when_user_owned(self):
        for name in ('postgres', 'qemu-system-x86_64', 'dpkg', 'mkfs.ext4', 'mount.ntfs',
                     'mysqld', 'redis-server', 'borg', 'systemd', 'gnome-shell', 'containerd-shim-runc-v2'):
            info=self.info();info.update(exe='/usr/bin/'+name,cmdline=name)
    ...
    def test_pid_one_ancestors_kernel_and_uninterruptible_io_are_protected(self):
        for change in ({'pid':1},{'pid':999},{'flags':0x200000},{'state':'D'}):
            info=self.info();info.update(change)
...
class TerminationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.holds=True;self.receipts=[];self.events=[]
        self.snapshot=Mock(side_effect=lambda pid: copy.deepcopy(self.info))
        self.safe=Mock(return_value=False)
        self.save=Mock()
        self.now=0
        self.patches=[patch.object(w.os,'pidfd_open',return_value=50),patch.object(w.os,'close'),
    ...
    def execute(self):
        return w.terminate_blocker(self.record,snapshot=self.snapshot,protected={1},
            still_holds=lambda pid,start:self.holds,safe_stop=self.safe,container_safe=lambda info:False,
            save=self.save,receipts=self.receipts,emit=lambda *a,**k:self.events.append((a,k)),
            clock=lambda:self.now,sleep=self.sleep,term_seconds=.5)
    ...
    def test_unknown_ordinary_escalates_after_bounded_grace_and_verifies(self):
        def release(fd,sig):
            if sig==signal.SIGKILL:self.holds=False
        self.send.side_effect=release
        self.execute()
        self.assertEqual([x.args[1] for x in self.send.call_args_list],[signal.SIGTERM,signal.SIGKILL])
        self.assertGreaterEqual(self.now,.5)

    def test_reused_pid_receives_no_signal(self):
        self.info['start']='99';self.execute();self.send.assert_not_called()
        self.assertFalse(self.receipts)

    def test_released_reference_does_not_kill_living_process(self):
        self.holds=False;self.execute();self.send.assert_not_called()

    def test_exec_into_critical_program_before_force_is_refused(self):
        self.send.side_effect=lambda *a:self.info.update(exe='/usr/bin/postgres',cmdline='postgres')
        with self.assertRaises(tx.Refusal):self.execute()
        self.send.assert_called_once_with(50,signal.SIGTERM)
    ...
    def test_critical_supported_stop_never_sends_signals(self):
        ...
        def shutdown(*a):self.holds=False;return True
        self.safe.side_effect=shutdown
        self.execute();self.send.assert_not_called();self.safe.assert_called_once()
    ...
    def test_control_process_cannot_use_even_safe_stop_callback(self):
        self.record['pid']=1;self.info['pid']=1
        with self.assertRaises(tx.Refusal):self.execute()
        self.safe.assert_not_called();self.send.assert_not_called()
...
class LiveOwnedProcessTests(unittest.TestCase):
    def test_real_unknown_blocker_term_then_force_only_owned_fixture(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            file=Path(directory)/'fixture';file.write_text('test')
            ...
            child=subprocess.Popen([sys.executable,'-c',code,str(file)],stdout=subprocess.PIPE,text=True)
            try:
                ...
                w.terminate_blocker({'kind':'fd','pid':child.pid,'start':info['start'],'path':str(file)},
                    protected={1,os.getpid()},still_holds=holds,safe_stop=lambda *a:False,
                    container_safe=lambda i:False,save=lambda:None,receipts=receipts,emit=lambda *a,**k:None,
                    term_seconds=.05)
...
class LockLockContractTests(unittest.TestCase):
    def test_legacy_or_missing_required_transaction_stage_is_not_ready(self):
        run=Mock(return_value=subprocess.CompletedProcess([],0,'LoadState=loaded\naag-sleep-recovery failure-recover',''))
        self.assertFalse(ll.stack_ready(run))

    def test_resume_requires_this_boot_actual_sleep_and_fresh_transaction(self):
        d={'boot_id':'boot','state':'COMPLETE','actual_sleep':True,'kernel_cycle':True,'started_monotonic':20}
        def read(p):return 'boot' if str(p).endswith('boot_id') else json.dumps(d)
        with patch.object(Path,'read_text',read):
            ...
            d['actual_sleep']=True;d['state']='RECOVERED'
            self.assertFalse(ll.resume_confirmed(19))
...
class LegacyLabelTests(unittest.TestCase):
    def test_invalid_volume_label_bytes_preserved_without_global_subprocess_change(self):
        ...
        with patch.object(production.subprocess,'run') as run:
            production.RecoveryCommands().run(['/usr/bin/lsblk','-J'],text=True,capture_output=True)
...
class StableAuditTests(unittest.TestCase):
    def test_changed_namespace_is_fully_reaudited_before_success(self):
        ...
        host.r=Mock()
        host.r.audit_three.side_effect=[{'AUDIT_COMPLETE':False,'gaps':[{'probe':'semantic-stability'}]},
    ...
    def test_permission_gap_is_never_promoted_to_complete(self):
        from aag_safe_suspend import production
        host=production.Host.__new__(production.Host);host.r=Mock()
        host.r.audit_three.return_value={'AUDIT_COMPLETE':False,'gaps':[{'probe':'permission'}]}
...
class ResumeHealthTests(unittest.TestCase):
    def result(self):
        return {'checks':{k:True for k in tx.HEALTH_CHECKS},'outcome':'NOT_ACCEPTED_REVIEW_REQUIRED'}

    def test_short_sleep_or_closed_lid_is_not_a_device_health_failure(self):
        result=self.result()
        result['checks'].update(sustained_hardware_sleep=False,residency_matches_cycle=False,lid_open_after_cycle=False)
    ...
    def test_every_actual_health_failure_remains_a_failure(self):
        for key in tx.HEALTH_CHECKS:
            result=self.result();result['checks'][key]=False
    ...
    def test_missing_or_new_failed_check_cannot_be_hidden(self):
        ...
        result=self.result();result['checks']['new_integrity_check']=False
...
class DeviceAdapterTests(unittest.TestCase):
    def setUp(self):
        ...
        self.out=self.root/'cycle-fixture';self.out.mkdir()
        self.device_tx={'directory':str(self.out),'invocation':'native','cursor':'cursor',
                        'before':{'boot_id':'boot','policy':{},'data':{'healthy':True}},
                        'cellular_before':{'healthy':True,'connection_uuid':'connection'}}
        self.job={'transaction':self.device_tx,'after':{},'invocation':'native','service_result':'success'}
        t=self.host.t;t.STATE=self.root;t.stale_pending=lambda:self.device_tx
        t.PENDING.touch();t.QUEUE.touch()
        self.state['native_invocation']='native';self.host.value=copy.deepcopy(self.state)
        t.read_json=lambda p:self.job
        t.write_json=Mock();t.verify_files=Mock();t.config=lambda:{};t.policy=lambda:{}
        t.core.snapshot=lambda:{};t.core.cellular=lambda:{'healthy':True,'connection_uuid':'connection'}
        t.core.data_state=lambda:{'healthy':True};t.core.BASE_SRC='baseline';t.core.loaded=lambda:'baseline'
        t.publish_result=Mock();t.block=Mock()
        ...
        self.run_patch=patch('aag_safe_suspend.production.run',return_value=subprocess.CompletedProcess([],0,'b false\n',''))
    ...
    def test_capture_failure_still_restores_policy_and_keeps_undo_evidence(self):
        ...
        with self.assertRaises(RuntimeError):production.Host.device_post(self.host)
    ...
    def test_data_health_failure_remains_a_real_failure(self):
        ...
        with self.assertRaises(tx.Refusal):self.host.verify_device()
    ...
    def test_healthy_closed_lid_wake_enters_single_owned_retry_path(self):
        ...
        with patch.object(production,'lid_ignored',return_value=False):
            with self.assertRaises(tx.Refusal):self.host.resume_terminal()
...
class QueueRecoveryTests(unittest.TestCase):
    ...
    def test_failed_queue_health_cannot_clear_transaction(self):
        ...
        with self.assertRaises(tx.Refusal):self.host.reconcile()
        self.assertNotEqual(self.state['state'],'IDLE')

    def test_device_post_exception_preserves_the_actual_first_error(self):
        from aag_safe_suspend import production
        self.state.update(native_invocation='prep',kernel_before={'success':0,'total_hw_sleep':0})
        self.host.device_post=Mock(side_effect=RuntimeError('policy endpoint disappeared'))
        with patch.object(production,'stats',return_value={'success':1,'total_hw_sleep':10000}):
            with self.assertRaises(RuntimeError):self.host.native_post()
