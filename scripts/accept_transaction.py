...
def transition_pending():
    ...
    try:
        ...
        if p.stdout.strip() != "b false":
            return True
        return any(
            production.properties(unit)["ActiveState"] not in ("inactive", "failed")
...
def execute(report: Path, inject_stale=True):
    ...
    if runtime.exists():
        raise RuntimeError("existing acceptance override requires review")
    for unit in (
        production.V2,
        production.NATIVE,
        production.VERIFY,
        ...
        if p["ActiveState"] not in ("inactive", "failed") or p.get("Job", "").strip() not in (
            "",
            "0",
        ):
    ...
    if prior_alarm and int(prior_alarm) > time.time():
        raise RuntimeError("existing future RTC alarm must be preserved")
    clones = host.clone_inventory()
    audit = host.clone_audit(clones)
    if not audit.get("AUDIT_COMPLETE"):
        (report / "acceptance-blockers.json").write_text(
            json.dumps(host.enrich(audit.get("records", [])), indent=2)
        )
    ...
    (report / "acceptance-initial-blockers.json").write_text(
        json.dumps(host.enrich(audit.get("records", [])), indent=2)
    )
    ...
    result.update(
        clones_before=clones, stats_before=production.stats(), thermal_before=thermal["state"]
    )
    (report / "inhibitors-acceptance-before.txt").write_text(
        production.run(["systemd-inhibit", "--list", "--no-pager"]).stdout
    )
    fixture = None
    try:
        with host.r.state_lock():
            host.value = host.r.load_state()
        if host.value.get("state") != "IDLE":
            host.reconcile()
        ...
        if mounts and host.cfg.get("notification_user"):
            ...
            if not pid.isdecimal():
                raise RuntimeError(
                    "test cwd fixture did not start: " + fixture.stderr.read()[-500:]
                )
            result["ordinary_blocker_fixture"] = {
                "pid": int(pid),
                "cwd": mounts[0],
                "writes": False,
            }
        if inject_stale:
            with host.r.state_lock():
                if host.r.load_state().get("state") != "IDLE":
                    raise RuntimeError("acceptance requires reconciled IDLE state")
                host.value = transaction.new(host.r.boot_id(), "acceptance-expired-owner")
                transaction.fail(
                    host.value,
                    "ACCEPTANCE_STALE_FENCE",
                    "synthetic stale state; no resources changed",
                )
        ...
        print(
            "ACCEPTANCE_BEGIN: normal suspend; RTC alarm armed only after successful preparation",
            flush=True,
        )
        ...
        while time.monotonic() < deadline:
            time.sleep(2)
            with host.r.state_lock():
                value = host.r.load_state()
            stage = (value.get("state"), value.get("stage"), value.get("reason"))
            if stage != last:
                print("ACCEPTANCE_STAGE " + json.dumps(stage), flush=True)
                last = stage
            current = production.stats()
            if (
                current["success"] > result["stats_before"]["success"]
                ...
                result.update(
                    ...
                    terminal=terminal,
                    status="PASS"
                    if terminal.get("actual_sleep") and terminal.get("state") == "COMPLETE"
                    else "FAIL",
                )
                break
            ...
            if (
                fresh
                and value.get("state") == "FAILURE_PENDING"
                and production.properties(production.FAILURE)["ActiveState"]
                in ("inactive", "failed")
            ):
                result.update(status="FAIL", failure=value, stats_after=current)
                break
            idle_units = all(
                production.properties(unit)["ActiveState"] in ("inactive", "failed")
                for unit in (production.V2, production.NATIVE, production.FAILURE)
            )
            if (
                value.get("state") == "IDLE"
                and value.get("last_terminal") == "RECOVERED"
                and idle_units
                and current["success"] == result["stats_before"]["success"]
            ):
                result.update(
                    status="FAIL",
                    failure="suspend aborted and safely recovered",
                    stats_after=current,
                )
                break
        else:
            result.update(status="TIMEOUT", failure=value, stats_after=production.stats())
    finally:
        ...
        if pending:
            result.update(
                status="FAIL", cleanup="deferred: active transition retains timed wake safety"
            )
        else:
        ...
        for name, args in [
            (
                "acceptance-journal.txt",
                ["journalctl", "--since", "@" + str(int(start)), "--no-pager", "-o", "short-iso"],
            ),
